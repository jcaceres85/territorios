...
class Search
{
    ...
    public static function get_search_by_geovisor_id($geovisor_id)
    {
		...
		$sql_query.= "INNER JOIN layers_attribute la ON (s.attribute=la.attribute AND s.layer_id=la.layer_id) ";
    ...
    public static function get_query_fields_string($geoserver_layer_id)
    {
    	...
    	$select_fields = ($hasGid?'':'gid,') . substr($select_fields, 0, strlen($select_fields) - 1);
